...
urlpatterns = [
    ...
    # ── Notes SYNC (оригінальний sync код — НЕ змінюємо) ─────────────────────
    ...
    #
    # Груповий чат — HTTP endpoint (рендерить HTML-сторінку зі скриптом).
    # Реальний WebSocket: ws://host/ws/groups/<pk>/chat/ → hello_project/routing.py
