...
Детальні коментарі до ORM: ../lesson_Django_ORM_Database/notes_project/notes_app/models.py
...
# ─────────────────────────────────────────────────────────────────────────────
# CHAT MESSAGE (WebSocket / Django Channels)
# ─────────────────────────────────────────────────────────────────────────────
...
class ChatMessage(models.Model):
    ...
    Навчальна роль:
    ───────────────
    Ця модель зберігає ВСІ повідомлення чату в БД.
    Завдяки цьому:
      1. Нові учасники бачать історію (останні 50 повідомлень при підключенні)
      2. Після перезавантаження сторінки повідомлення не губляться
      3. Consumer може зберігати і завантажувати повідомлення через ORM

    Зв'язки:
      - group: до якої групи належить повідомлення (CASCADE — група видалена → чат очищено)
      - author: хто написав (CASCADE — user видалений → його повідомлення видалені)

    Індекс (group, timestamp):
      Єдиний query pattern у цьому проєкті —
      "останні N повідомлень для групи X ORDER BY timestamp".
      Складений індекс покриває цей запит повністю.
    """
    group = models.ForeignKey(
        Group,
    ...
    def __str__(self):
        return f"[{self.group.name}] {self.author.username}: {self.content[:40]}"
